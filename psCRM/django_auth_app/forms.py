...
#from sundog.models import AreaOfInterest, CommunityOfInterest
import enums
import string

"""
	Form for signup of the user
"""
MIN_LENGTH = 6
...
class ProfileForm(forms.ModelForm):
    ...
    gender = forms.TypedChoiceField(
                 choices=enums.GENDER_CHOICES, widget=forms.RadioSelect, coerce=int, required=False
    )
    # state_of_interest = forms.ChoiceField(required=False, widget=forms.Select(attrs={'class':'form-control','required':'','onChange':'updateAreasOfInterest(this)'}), choices=enums.US_STATES_I)
    # area_of_interest = forms.ModelChoiceField(required=False, queryset=AreaOfInterest.objects.none(), empty_label="Select an area of interest...", widget=forms.Select(attrs={'class':'form-control','required':'','onChange':'updateCommunitiesOfInterest(this)'}))
    # communities_of_interest = forms.ModelMultipleChoiceField(required=False, queryset=CommunityOfInterest.objects.none(), widget=forms.SelectMultiple(attrs={'class':'chosen-select','required':'','data-placeholder':"Choose communities of interest..."}))

    class Meta:
        model = UserProfile
        fields = ('timezone', 'phone_number', 'birthday', 'middle_name', 'first_name', 'last_name',
                  'zip_code', 'address', 'city', 'state', 'country')
                 # 'state_of_interest', 'area_of_interest', 'communities_of_interest')
