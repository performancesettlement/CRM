...
from models import MyFile, FileStatus, Client, Tag, ClientType
import services
from haystack.forms import SearchForm
from constants import RADIO_FILTER_CHOICES, SHORT_DATE_FORMAT
...
class FileSearchForm(SearchForm):
    ...
    status = forms.ModelChoiceField(required=False, empty_label="Filter by file status..",
                                    ...
                                    widget=forms.Select(attrs={'class':'input-sm form-control input-s-sm inline', 'onChange': 'updateSearch()'}))
...
class FileCustomForm(forms.ModelForm):
    ...
    current_status = forms.ModelChoiceField(required=True, queryset=FileStatus.objects.none(),
                                            widget=forms.Select(attrs={'class':'form-control', 'onChange': 'updateCompleted(this)'}))
    client = forms.ModelChoiceField(required=True, queryset=Client.objects.all(),empty_label="Select a client...",
                                            widget=forms.Select(attrs={'class':'chosen-select', 'data-placeholder': 'Select a client...'}))
    tags = forms.ModelMultipleChoiceField(required=False, queryset=Tag.objects.all(),
                                    widget=forms.SelectMultiple(attrs={'class':'chosen-select', 'data-placeholder': 'Click to display...'}))
    quoted_date = forms.DateField(required=False, widget=forms.DateInput(format=SHORT_DATE_FORMAT, attrs={'placeholder': 'mm/dd/yyyy','class':'form-control'}))
    invoice_date = forms.DateField(required=False, widget=forms.DateInput(format=SHORT_DATE_FORMAT, attrs={'placeholder': 'mm/dd/yyyy','class':'form-control'}))
    quoted_price = forms.DecimalField(required=False, widget=forms.NumberInput(attrs={'min': '0','class':'form-control'}))
    invoice_price = forms.DecimalField(required=False, widget=forms.NumberInput(attrs={'min': '0','class':'form-control'}))

    class Meta:
        model = MyFile
        exclude = ('participants', 'messages', 'creator_user_username', 'creator_user_full_name',  'created_time', 'last_update_time',
                   'last_update_user_username', 'last_update_user_full_name', 'file_status_history')


class ClientForm(forms.ModelForm):
    client_type = forms.ModelChoiceField(queryset=ClientType.objects.all(), empty_label="Select a client type...",
                                    widget=forms.Select(attrs={'class': 'chosen-select', 'data-placeholder': 'Select a client type...'}))
    ...
    def clean(self):
        ...
        if name:
            ...
            pk = services.check_client_exists(name)
            if pk:
        ...
        if identification:
            ...
            pk = services.check_client_exists_by_identification(identification)
            if pk:
