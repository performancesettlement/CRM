...
def get_user_status_permissions(user):
    try:
        ...
        status_perms = [str(x)[len(permission_prefix):].upper() for x in user.get_all_permissions()
...
def get_status_list_by_user(user, from_admin=False):
    ...
    try:
        if user.is_superuser:
            ...
            if from_admin:
                results = FileStatus.objects.filter(name__in=permissions_status_list).order_by('order', 'name')
            else:
                results = FileStatus.objects.filter(active=True, name__in=permissions_status_list).order_by('order', 'name')
    except Exception, e:
...
def get_participants_options(file_id, current_status, current_participants):
    ...
    try:
        ...
        ids = map(lambda x: x.id, current_participants) if current_participants else []
...
def check_client_exists_by_identification(ident):
    if ident is None:
        return None
...
def upload_import_file(import_excel, user):
    errors = []
    try:
        ...
        if first_sheet.nrows < 2:
            ...
            for row_idx in range(1, first_sheet.nrows):
                ...
                if file_cell.value:
                    client_id = check_client_exists(str(file_cell.value).upper().strip())
                    if not client_id:
        ...
        if not errors:
            # save import history
            save_import_history(import_excel, user)
            # save to db
            for my_file in files:
                my_file.stamp_created_values(user)
                my_file.save()
                file_status = FileStatusHistory()
                file_status.create_new_file_status_history(None, my_file.current_status, user)
...
def upload_import_client(import_excel, user):
    errors = []
    try:
        book = open_workbook(file_contents=import_excel.read())
        # get the first worksheet
        ...
        if first_sheet.nrows < 2:
            ...
            for row_idx in range(1, first_sheet.nrows):
                ...
                if file_cell.value:
                    ...
                    client_id = check_client_exists(cell_name)
                ...
                if file_cell.value:
                    ...
                    client_ident = check_client_exists_by_identification(cell_ident)
...
def save_import_history(import_excel, user):
    try:
        dt_now = datetime.now()
...
def check_file_history_checksum(checksum):
    result = False
    try:
        history = FileImportHistory.objects.get(import_checksum=checksum)
        result = True
