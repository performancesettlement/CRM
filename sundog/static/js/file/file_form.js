...
$(document).ready(function(){
    ...
    $('#file-edit-check-concurrency').on('click', function(event) {
        event.preventDefault();
        var form = $('#edit-form');
        var formData = form.serializeArray();
        $.post(file_edit_url, formData, function(response) {
            if (response.form_errors) {
                form_errors = response.form_errors;
                showErrorPopup(form_errors);
            } else {
                if (response.noChanges) {
                    refreshScreen();
                } else {
                    var changedBy = response.changedBy;
                    var message = "The user '" + changedBy + "' has changed this file data. Do you want to override his changes or refresh the screen?";
                    $('#edit-file-concurrency-check-message').html(message);
                    $('#edit-file-concurrency-check-modal').modal('show');
                }
            }
        }, "json");
    });

    $('.refresh-screen').on('click', function(event) {
        refreshScreen();
    });

    $('#edit-file-submit').on('click', function(event) {
        var form = $('#edit-form');
        var formData = form.serializeArray();
        formData.push({name: 'override', value: true});
        $.post(file_edit_url, formData, function(response) {
            refreshScreen();
        });
    });
