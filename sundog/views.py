...
def _get_users_by_company():
    ...
    for user in list(User.objects.prefetch_related('userprofile').prefetch_related('userprofile__company').all()):
        if user.userprofile.company and user.userprofile.company.company_id not in users_by_company:
            if user.userprofile.company.company_id not in users_by_company:
                users_by_company[user.userprofile.company.company_id] = []
            users_by_company[user.userprofile.company.company_id].append({'id': str(user.id), 'name': user.get_full_name()})
